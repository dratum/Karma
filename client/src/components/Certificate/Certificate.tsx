...
export default function Certificate({
  ...
  async function buyCertificateHandler(
    sum: string,
    image: string
  ): Promise<void> {
    try {
  ...
  return (
    <>
      <div
        className={
          "rounded-md bg-white p-3 text-left hover:scale-105 transition duration-300 pl-8 mb-10"
        }
      >
        ...
        >
          <div className='bg-white p-6 rounded-md'>
            <p className='mb-4 text-lg'>
              ДЕЛАЙ ЛЮДЯМ ДОБРО И ОНО К ТЕБЕ ВЕРНЕТСЯ. <br />
              СЕРТИФИКАТ ТВОЙ, ДОБРЫЙ ЧЕЛОВЕК. <br />
              ПРОВЕРЬ СВОЮ ПОЧТУ.
            </p>
      ...
      {buyFailure && (
        ...
        >
          <div className='bg-white p-6 rounded-md content-center '>
            <p className='mb-4 text-lg'>
              К СОЖАЛЕНИЮ, НА ПРИОБРЕТЕНИЕ ДАННОГО СЕРТИФИКАТА НЕ ДОСТАЕТ
              БАЛЛОВ. <br />
              НЕ ОТЧАЙВАЙСЯ - ЭТО ЛИШЬ ПОВОД СДЕЛАТЬ БОЛЬШЕ ДОБРЫХ ДЕЛ!
            </p>
            <div className='flex justify-center'>
              <button
                onClick={handlePopupClose}
